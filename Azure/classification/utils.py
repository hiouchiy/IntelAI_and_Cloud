...
def display_images(directory, numOfImages = 5):
    files = []
    for root, directories, filenames in os.walk('breeds/'):
        for filename in filenames:
            if os.path.splitext(filename)[1] == ".jpg":
                files.append(os.path.join(root,filename))
    indicies = random.sample(range(len(files)), numOfImages * numOfImages)    
    ...
    for i,ax in enumerate(axes.flat):
        ...
        imgplot = mpimg.imread(files[indicies[i]], 0)
        ax.imshow(imgplot)
        ax.text(10,20,files[indicies[i]].split("/")[-2], fontdict={"backgroundcolor": "black","color": "white" })
        ax.axis('off')
